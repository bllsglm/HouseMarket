...
import { ListingProp } from './ListingItem'
...
const MyListings = () => {
  ...
  useEffect(() => {
    const fetchMyListings = async () => {
      ...
      const q = query(
        ListingRef,
        where('userRef', '==', firebaseAuth.currentUser?.uid),
        orderBy('timestamp', 'desc')
      )
      const myListingSnap = await getDocs(q)

      const myAllListings: { id: string; data: ListingProp; time: string }[] =
        []
      myListingSnap.forEach((doc) => {
        const time = doc.data().timestamp.toDate().toLocaleDateString()
        const data = doc.data() as ListingProp
        return myAllListings.push({ id: doc.id, data, time })
      })
  ...
  return (
    <>
      <h2 className="m-4 p-4 font-extrabold text-2xl text-opacity-50 border-b-2 border-gray-950">
        My Listings
      </h2>
      <div className="grid grid-cols-1 lg:grid-cols-2 min-w-[280px]">
        {mylistings?.length === 0 ? (
          <h1 className="text-white text-center text-2xl font-extrabold bg-rose-500 rounded-lg m-4 p-4 shadow-lg shadow-slate-800">
          ...
          <>
            {mylistings?.map((mylisting, index) => (
              <Link
                to={`/category/${mylisting.data.type}/${mylisting.id}`}
                key={index}
              >
                <div className="flex flex-col sm:flex-row  shadow-2xl gap-4 bg-slate-100 m-2 p-4 rounded-2xl hover:bg-green-100 cursor-pointer">
                  ...
                  <div className="flex flex-1 flex-col gap-2 ">
                    ...
                    <p className="text-green-500 font-extrabold">
                      ...
                      $
                      {Number(
                        mylisting.data.offer
                          ? mylisting.data.discountedPrice
                          : mylisting.data.regularPrice
                      ).toLocaleString()}
                    </p>
